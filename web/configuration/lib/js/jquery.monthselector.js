...
(function($) {
    ...
    $.ms = function(elem, options, arg) {
        ...
        for(var j=0; j < $.ms.monthSelectArray.length; j++) {
            var $innerDiv   = $("<option />").addClass("month_entry ui-state-default").html($.ms.monthSelectArray[j]).val($.ms.monthSelectValues[j]);
        ...
        var $leftDiv   = $("<div />").addClass("ui-state-default ui-corner-all ms_button_left").attr("id", $.ms.settings.id_button_left+$.ms.elemId);
        var $imgLeft   = $("<img />").attr("src", "images/16x16/arrow_left.png");
        $leftDiv.append($imgLeft);
        var $middleDiv = $("<div />").addClass("ms_button_middle ui-state-default ui-corner-all").attr("id", $.ms.settings.id_button_middle+$.ms.elemId);
        var $rightDiv  = $("<div />").addClass("ms_button_right ui-state-default ui-corner-all").attr("id", $.ms.settings.id_button_right+$.ms.elemId);
        var $imgRight  = $("<img />").attr("src", "images/16x16/arrow_right.png");
        $rightDiv.append($imgRight);
        var $tbl = $("<div />").addClass("ms_tbl_container");
        var $row = $("<div />").addClass("ms_tbl_row_container").attr("id", $.ms.settings.id_tbl_row_container+$.ms.elemId);
        $tbl.append($row);
         ...
         */
        $('#' + $.ms.settings.id_button_middle+$.ms.elemId).bind("click", function() {
            var destination = $('#'+$.ms.settings.id_button_middle+$.ms.elemId).offset();
    ...
    _fillMonthArray = function() {
         ...
         * 12 month per year * 2 years to the past (24) and
         * 12 month per year * 2 years to the future (24) and
         * the current month (1)
         */
